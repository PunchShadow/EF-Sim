...
def createRecGraph(*arg):
    ...
    if(arg[0] == 2):
        for i in range(node_num):
            G.add_edge('G',i, gid=0, energy=2.0, time=1.0)
            row = i//arg[2]
            col = i%arg[2]
            if(col+1 < arg[2]):
                G.add_edge(i, i+1, gid=1, energy=0.5, time=0.1)
            if(row+1 < arg[1]):
                G.add_edge(i, i+arg[2], gid=2, energy=0.5, time=0.1)
    ...
    # 3D matrix
        ...
        for i in range(node_num):
            ...
            if(col+1 < arg[2]):
                G.add_edge(i, i+1, gid=1, energy=0.5, time=0.1)
            if(row+1 < arg[1]):
                G.add_edge(i, i+arg[2], gid=1, energy=0.5, time=0.2)
            if(height+1 < arg[3]):
                G.add_edge(i, i+plane_num, gid=1, energy=0.5, time=0.3)
...
def eyeriss(config): #12*14
    ...
    for j in range(14):
        for i in range(12):
            v=i*14+j
            G.add_edge('G', v, gid=i, energy=0.5, time=0.1) # filter edge
